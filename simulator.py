...
class Simulator:
    ...
    def mean_iou(self) -> float:
        ...
        return (self.road_iou + self.vegetation_iou + self.sky_iou) / 3.0
    
    def step(self, action: Action) -> dict:
        """Execute one step with given action."""
        info = {"msg": "", "iou_improvement": 0.0}
        self.repeating_penalty = 0.0
        
        # Penalize repeated configuration actions (idempotent but wasteful)
        if (action.action == self.last_action and 
            action.action not in ["run_training_epoch", "early_stop_training"]):
            self.repeating_penalty = -0.3
            info["msg"] = "⚠️ Repeated configuration action (already enabled)."
        ...
        if action.action == "adjust_learning_rate":
            ...
            info["msg"] = "✓ Learning rate optimized to 0.001. Instability resolved."
            
        elif action.action == "enable_augmentation":
            self._enable_augmentation()
            info["msg"] = "✓ Data augmentation enabled (rotation, flip, color jitter). Generalization improved."
            
        elif action.action == "increase_regularization":
            self._increase_regularization()
            info["msg"] = "✓ L2 regularization increased to 0.0005. Overfitting risk reduced."
            
        elif action.action == "switch_optimizer":
            self._switch_optimizer()
            info["msg"] = "✓ Switched to AdamW optimizer. Better convergence expected."
            
        elif action.action == "enable_class_balancing":
            self._enable_class_balancing()
            info["msg"] = "✓ Focal loss with class reweighting enabled. Sky class will receive gradients."
            
        elif action.action == "reduce_batch_size":
            self._reduce_batch_size()
            info["msg"] = "⚠️ Batch size reduced to 16. More memory efficient but slower convergence."
            
        elif action.action == "early_stop_training":
            self.early_stopped = True
            info["msg"] = "⊗ Training stopped. Epoch budget preserved."
    ...
    def _epoch_with_instability(self):
        """Training degrades without proper learning rate."""
        decay = 0.05 + (random.random() * 0.03)  # Unstable
        self.road_iou = max(0.10, self.road_iou - decay)
        self.vegetation_iou = max(0.00, self.vegetation_iou - decay * 0.5)
        self.train_loss += random.uniform(0.1, 0.3)
        self.val_loss += random.uniform(0.1, 0.3)
    
    def _epoch_with_plateau(self):
        """Minimal improvement after convergence."""
        improvement = random.uniform(0.001, 0.003) * (1.0 + self.augmentation_level * 0.1)
        self.road_iou += improvement * 0.5
        self.vegetation_iou += improvement * 0.3
        if self.class_balancing:
            self.sky_iou += improvement * 2.0
        self.train_loss *= 0.98
        self.val_loss *= 0.99
    
    def _epoch_normal_training(self):
        """Normal training with learning curve dynamics."""
        base_improvement = 0.05
        
        # Learning curve starts steep, then plateaus
        ...
        # Apply improvements
        self.road_iou += improvement * 0.7
        self.vegetation_iou += improvement * 0.4
        
        # Sky class only learns with class balancing
        if self.class_balancing:
            self.sky_iou += improvement * 0.5
        else:
            # Stays at 0 without balancing
            self.sky_iou = 0.0
        ...
        if self.total_epochs_trained >= 5 and improvement < 0.01:
    ...
    def _generate_report(self) -> str:
        ...
        issues = []
        if self.instability_flag:
            issues.append("  ⚠️  Training instability - suboptimal learning rate (try 'adjust_learning_rate')")
        if self.overfitting_flag:
            issues.append("  ⚠️  Overfitting detected - validation loss diverging (try 'increase_regularization')")
        if self.training_plateau:
            issues.append("  ℹ️  Training plateau - gains diminishing after convergence")
        if self.class_imbalance_detected:
            issues.append("  ⚠️  Sky class not learning - severe class imbalance (try 'enable_class_balancing')")
