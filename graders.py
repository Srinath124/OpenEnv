...
class EasyTaskGrader:
    ...
    def score(state: dict) -> float:
        ...
        if mean_iou >= EasyTaskGrader.TARGET_IOY:
            return 1.0
        
        # Linear progression: 0 IoU -> 0 score, 0.50 IoU -> 1.0 score
        return max(0.0, mean_iou / EasyTaskGrader.TARGET_IOY)
...
class MediumTaskGrader:
    ...
    def score(state: dict) -> float:
        ...
        Overfitting causes 50% score penalty.
        """
        mean_iou = state.get("mean_iou", 0.0)
        overfitting = state.get("overfitting", False)
        
        # Base score from IoU
        base_score = min(1.0, mean_iou / MediumTaskGrader.TARGET_IOY)
        
        # Harsh penalty for overfitting
        if overfitting:
            base_score *= 0.5
...
class HardTaskGrader:
    ...
    TARGET_MEAN_IOY = 0.65
    TARGET_SKY_IOY = 0.50
    MIN_COMPUTE_REMAINING = 5  # minutes
    ...
    def score(state: dict) -> float:
        ...
        - Efficiency: 15% (requires >= 5 min compute remaining)
        - Stability: 15% (no instability)
        """
        mean_iou = state.get("mean_iou", 0.0)
        sky_iou = state.get("sky_iou", 0.0)
        instability = state.get("instability", True)
        compute_remaining = state.get("remaining_compute", 0)
        
        score = 0.0
        
        # Component 1: IoU progress (40% weight)
        iou_component = min(1.0, mean_iou / HardTaskGrader.TARGET_MEAN_IOY)
        ...
        sky_component = min(1.0, sky_iou / HardTaskGrader.TARGET_SKY_IOY)
        score += sky_component * 0.30
        
        # Component 3: Efficiency (15% weight)
        # Needs to complete task without exhausting compute budget
        if compute_remaining >= HardTaskGrader.MIN_COMPUTE_REMAINING:
            efficiency_component = 1.0
        else:
            efficiency_component = max(0.0, compute_remaining / HardTaskGrader.MIN_COMPUTE_REMAINING)
        score += efficiency_component * 0.15
        
        # Component 4: Stability (15% weight)
        # Unstable training is unacceptable in production
        stability_component = 0.0 if instability else 1.0
