...
Ensures strict validation and bounded ranges for all values.
"""
...
class Reward(BaseModel):
    ...
    value: float = Field(
        0.0, 
        description="Total reward signal (sum of components minus penalties). Bounded [-1, 1]."
    )
...
class Observation(BaseModel):
    ...
    @field_validator('mean_iou', 'road_iou', 'vegetation_iou', 'sky_iou')
    @classmethod
    def bound_iou(cls, v):
        """Ensure all IoU values stay in [0, 1] range."""
